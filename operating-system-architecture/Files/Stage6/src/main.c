...
int main(int argc, char* argv[]) {
	...
	int empty_message_len = strlen(empty_message);
	for(int i =0; i < 5; ++i){
		int write_result = -2;
		if(rand() % 2 == 0) {
    		write_result = write(file_descriptor, message, message_len);
		}else{
    		write_result = write(file_descriptor, empty_message, empty_message_len);
		}
